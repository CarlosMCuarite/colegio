...
import { authenticate, isDocente, isStaff } from '../middleware/auth';
...
// ── POST /cursos ──────────────────────────────────────────────────────────────
router.post('/', isStaff, auditar({ modulo: 'CURSOS', accion: AuditoriaAccion.CREAR }), async (req, res) => {
...
router.post('/plantilla-por-defecto', isStaff, auditar({ modulo: 'CURSOS', accion: AuditoriaAccion.CREAR }), async (req, res) => {
...
router.post('/eliminar-lote', isStaff, auditar({ modulo: 'CURSOS', accion: AuditoriaAccion.ELIMINAR }), async (req, res) => {
...
// ── PATCH /cursos/:id ──────────────────────────────────────────────────────────
router.patch('/:id', isStaff, auditar({ modulo: 'CURSOS', accion: AuditoriaAccion.ACTUALIZAR, getRecursoId: r => r.params.id }), async (req, res) => {
...
router.delete('/:id', isStaff, auditar({ modulo: 'CURSOS', accion: AuditoriaAccion.ELIMINAR, getRecursoId: r => r.params.id }), async (req, res) => {
