...
// Antes este archivo era `export { default } from '../../padre/horarios/page'`,
// y esa página tiene `allowedRoles={['PADRE']}` — por eso un Docente que
// entraba a "Mis Horarios" caía directo a /unauthorized. El backend
// (GET /horarios) YA filtra correctamente por docente (sus propias clases/
// aulas asignadas), así que solo hacía falta esta página propia con los
// roles correctos; no se toca el backend.
export default function HorariosDocentePage() {
  ...
  return (
    <DashboardLayout title="Mis Horarios" allowedRoles={['DOCENTE','AUXILIAR','TUTOR','COORDINADOR','PSICOLOGO','ENFERMERIA']}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {items.map((h: any) => (
                  <div key={h.id} style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '0.4rem 0', borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    {h.seccion?.nombre && <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{h.nivelGrado?.nombre} "{h.seccion.nombre}"</span>}
                  </div>
                ))}
              </div>
            </div>
