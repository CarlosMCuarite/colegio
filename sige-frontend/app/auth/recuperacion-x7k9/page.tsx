...
export default function RecuperacionAdminPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/auth/recuperacion-admin`, {
        method: 'POST',
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: '2.5rem', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
            ...
            <h1 style={{ fontSize: '1.15rem', fontWeight: 800, color: '#111827', marginBottom: 6 }}>Recuperación de administrador</h1>
            <p style={{ color: '#6b7280', fontSize: '0.82rem', margin: 0 }}>
              Solo para cuentas ADMINISTRADOR / SUPERADMIN. Necesitas el código de recuperación.
            </p>
          </div>
